...
#     """
#     像处理endpart之中的被展开函数一样移动和修改被调用函数的代码，
#     （除了代码移动以外，一般是函数调用的地方需要替换成相应的return变量）
#     :param partlist: list，倒序的部分区间
#     :param pathlist: list,路径，包含所有基本块
#     :return:
#     """
#     blocknamelist = []
#     for i in pathlist:
...
#                 fun_ast_map[callerfunname] = loop_list[0]
#                 find_loopid(loop_list[0], varlist)
#                 next_ast = loop_move(loop_list[0], linenum, calledele, re_po_flag)
#                 fun_ast_map[calledfunname] = next_ast
#                 if part == partlist[-2]:
#                     break
#                 else:
#                     curind = partlist.index(part)
#                     tunc_list = partlist[curind + 1:]
#                     # 非loop移动
#                     for t in tunc_list:
#                         print("tunc_list....1")
#                         print(tunc_list)
...
#                         if len(templist) > 0:
#                             linenum = templist[0]
#                             calledele = pathlist[startind - 2]
#                             mvflag = "reverse"
#                             next_ast = notloop_move(linenum, next_ast, calledele,mvflag)
#                             fun_ast_map[calledfunname] = next_ast
#
#                 for ext1 in tempast.ext:
...
# def loop_same_source_sink(split_pathlist, pathlist, end_start_part, outfile, genfunname):
#     """
#     根据被调用函数的顺序将所有被调用函数移动进入主调函数中的loop中
#     :param startele:
#     :param list3:
#     :param start:
#     :param end:
#     :param split_pathlist:
#     :return:
#     """
...
#         bbname=i.get_name()
#         blocknamelist.append(bbname)
#     split_path_end(blocknamelist,endpart_list)
#     startpart_list = split_pathlist[ind + 1:]
#     fun_ast_map = {}
#     param_list = []
#     reverse_flag = "reverse"
#     loop_move_part_reverse_order(endpart_list, pathlist, fun_ast_map, reverse_flag, param_list)
#     positive_flag = "reverse"
#     if False in fun_ast_map.keys():
#         return
#     callerast = fun_ast_map[callerfunname]
#     # print("callerast...")
#     # print3488(callerast)
#     # print(callerast)
#     # print(callerast.show(showcoord=callerast.coord))
#     loop_move_part_positive_order(startpart_list, pathlist, callerast, positive_flag)
#     f_ast = fun_ast_map[callerfunname]
#
#     fundef = getfundef()
#     fundef.ext[0].decl.name = genfunname
#     fundef.ext[0].decl.type.type.declname = genfunname
...
#     fundef.ext[0].body.block_items = []
#
#     fundef.ext[0].body.block_items.append(f_ast)
#     c_file = open(outfile, "w")
#     c_file.write(generator.visit(fundef))
#     c_file.close()
# print("finally+code")
