...
from deal_return import deal_return, deal_return1, deal_void_return
from delete_fun import delete_fun
    ...
    for s in structs:
        if s.getAttribute("name") == ty:
            if s.getAttribute("type") == "Union":
                break
            if s.getAttribute("type") == "Enum":

                assign = get_assign()
                ass = get_assert()
                if type(newname) == c_ast.StructRef or type(newname) == c_ast.ArrayRef:
                    newname1 = generator.visit(newname)
                    rname1 = generator.visit(rname)

                    assign.lvalue.name = newname1
                    assign.rvalue.name = rname1
                    ass.args.exprs[0].left.name = newname1
                    ass.args.exprs[0].right.name = rname1
                ...
                rast.body.block_items.insert(record, assign)
                m_ast_len = len(mast.body.block_items)
                if funtype == 'void':
                    mast.body.block_items.append(ass)
                else:
                    mast.body.block_items.insert(m_ast_len - 1, ass)
                    ...
                    if ii.hasAttribute("level"):
                        ...
                        if isptr:
                            if ii.hasAttribute("ptr") and ii.hasAttribute("arr"):
                                ...
                                recur(structs, temprname, temnewname, subnewname, n_isptr, Type, rast, x, mast, funtype,
                                      rdepth)
                            elif ii.hasAttribute("arr"):
                                isptr1 = False
                                subname = ii.getAttribute("name")  # nest object's name
                                ...
                                arr_recur(structs, temprname, tempnewname, tempnewname1, arr_size, isptr1, Type, rast,
                                          record, mast, funtype, rdepth)
                                continue
                            else:  # no ptr
                                ...
                                recur(structs, temprname, tempnewname, temname, n_isptr, Type, rast, record, mast,
                                      funtype, rdepth)
                            ...
                            if ii.hasAttribute("ptr") and ii.hasAttribute("arr"):
                                ...
                                recur(structs, temprname, temnewname, temnewname, n_isptr, Type, rast, record, mast,
                                      funtype, rdepth)
                    ...
                    if ii.hasAttribute("level"):
                        ...
                        if isptr:
                            ...
                            if ii.hasAttribute("ptr") and ii.hasAttribute("arr"):
                                ...
                                arr_recur(structs, temprname, tempnewname, tempnewname, arr_size, isptr1, ty1, r_ast,
                                          record, m_ast, funtype, rdepth)
                                continue
            break
...
def self_com(ast, ast1, f, structs, filename, content):
    ...
    pp = open("../../../path/pycparser.txt")
    pline = pp.readline().strip()
    path = pline + line + '/pilot_src/Source/' + filename + '.c'
