...
def find_return(st, ret_name, gotost, funnamne,pst):
    ...
    # if type(st) == c_ast.If:
    #     for i in st:
    #         if type(i) == c_ast.Return:
    #             assign = c_ast.Assignment(op='=', lvalue=c_ast.ID(name=ret_name), rvalue=i.expr)
    #             if type(i.expr) == c_ast.FuncCall:
    #                 lineinfo = str(i.coord)
    #                 index = lineinfo.index(":")
    #                 rindex = lineinfo.rindex(":")
    #                 linenum = int(lineinfo[index + 1:rindex])
    #                 last_num = -1
    #                 inc_linenum = []
    #                 get_last_linenum(i, last_num, inc_linenum)
    #                 global_return_sta[funnamne][linenum] = [assign, inc_linenum[0]]
    #             if type(st) == c_ast.Compound:
    #                 st.block_items.remove(i)
    #                 st.block_items.append(assign)
    #                 st.block_items.append(gotost)
    #             elif type(st) == c_ast.If and i == st.iftrue:
    #                 tempst = c_ast.Compound(block_items=[assign, gotost])
    #                 st.iftrue = tempst
    #             elif type(st) == c_ast.If and i == st.iffalse:
    #                 tempst = c_ast.Compound(block_items=[assign, gotost])
    #                 st.iffalse = tempst
    #             else:
    #                 if type(st) == c_ast.Case or type(st) == c_ast.Default:
    #                     st.stmts.remove(i)
    #                     st.stmts.append(assign)
    #                     st.stmts.append(gotost)
    #                     continue
    #         else:
    #             find_return(i, ret_name, gotost, funnamne)
    # else:
    #     for child in st:
    #         if type(child) == c_ast.Return:
    #             assign = c_ast.Assignment(op='=', lvalue=c_ast.ID(name=ret_name), rvalue=child.expr)
    #             if type(child.expr) == c_ast.FuncCall:
    #                 lineinfo = str(child.coord)
    #                 index = lineinfo.index(":")
    #                 rindex = lineinfo.rindex(":")
    #                 linenum = int(lineinfo[index + 1:rindex])
    #                 last_num = -1
    ...
    #             else:
    #                 if type(st) == c_ast.Case or type(st) == c_ast.Default:
    #                     st.stmts.remove(child)
    #                     st.stmts.append(assign)
    #                     st.stmts.append(gotost)
    #                     continue
    #         else:
    #             find_return(child, ret_name, gotost, funnamne)
